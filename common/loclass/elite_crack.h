...
/**
----------------------------------------------------------------------------------------------------
Here are some pretty optimal values that can be used 
to recover necessary data in only eight auth attempts.
                                                                                           Bytes
// CSN                                        HASH1                                      recovered
...
/**
 *
 * @brief Same as `bruteforceFile`, if you don't care about the returned keytable.
 * 
 * (results only printed on screen)
 * @param filename
 * @return
 */
...
/**
 * @brief Same as `bruteforceFile`, but uses a an array of `dumpdata_t` instead
 * 
 * @param dump
 * @param dumpsize
 * @param keytable
...
/**
 * @brief 
 * Performs brute force attack against a `dumpdata_t` item, containing csn, cc_nr and mac.
 * 
 * This method calculates the hash1 for the CSN, and determines what bytes need to be bruteforced
 * on the fly.
 * 
 * If it finds that more than three bytes need to be bruteforced, it aborts.
 * 
...
/**
 * @brief From dismantling iclass-paper:
 * Assume that an adversary somehow learns the first 16 bytes of `hash2(K_cus)`, i.e., `y[0]` and `z[0]`.
 * Then he can simply recover the master custom key `K_cus` by computing `K_cus = ~DES(z[0], y[0])`.
 *
 *  Furthermore, the adversary is able to verify that he has the correct `K_cus` by
