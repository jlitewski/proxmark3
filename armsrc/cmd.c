...
static int reply_ng_internal(uint16_t cmd, int16_t status, const uint8_t *data, size_t len, bool ng) {
    ...
    // Add the (optional) content to the frame, with a maximum size of PM3_CMD_DATA_SIZE
    if (data && len) palloc_copy(txBufferNG.data, data, len);
...
static int receive_ng_internal(PacketCommandNG *rx, uint32_t read_ng(uint8_t *data, size_t len), bool usb, bool fpc) {
    ...
    if (bytes == 0) return PM3_ENODATA;

    if (bytes != sizeof(PacketCommandNGPreamble)) return PM3_EIO;
    ...
    if (rx->magic == COMMANDNG_PREAMBLE_MAGIC) { // New style NG command
        ...
        if (rx->ng) {
            ...
            uint64_t arg[3] = {0};
            if (length < sizeof(arg)) return PM3_EIO;
        ...
        if (bytes != sizeof(PacketCommandNGPostamble)) return PM3_EIO;
        ...
        if (rx->crc != COMMANDNG_POSTAMBLE_MAGIC) {
            ...
            if ((first << 8) + second != rx->crc) return PM3_EIO;
        }
        ...
        if (bytes != sizeof(PacketCommandOLD) - sizeof(PacketCommandNGPreamble)) return PM3_EIO;
...
int receive_ng(PacketCommandNG *rx) {
    if (usb_poll_validate_length()) // Check if there is a packet available
        return receive_ng_internal(rx, usb_read_ng, true, false);

#ifdef WITH_FPC_USART_HOST
    if (usart_rxdata_available() > 0) // Check if there is a FPC packet available
