...
void ht2_crack2(uint8_t *nrar_hex) {
    ...
    uint8_t *e_response = (uint8_t*)palloc(1, 32);
    lf_hitag_crack2_t *c2 = (lf_hitag_crack2_t *)palloc(1, sizeof(lf_hitag_crack2_t));

    if(e_response == nullptr || c2 == nullptr) {
        if(PRINT_ERROR) Dbprintf("Unable to allocate memory! Aborting...");
        palloc_free(e_response);
        palloc_free(c2);
        reply_ng(CMD_LF_HITAG2_CRACK_2, PM3_EMALLOC, nullptr, 0);
        return;
    }
